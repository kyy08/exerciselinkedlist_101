...
bool CircularLinkedList::search(int rollno, Node** previous, Node** current) {
	...
	*current = LAST->next;
	while (*current != LAST) {
		if (rollno == (*current)->Mohamadwahyu) {
			return true;
		}
		*previous = *current;
		*current = (*current)->next;
	}
	if (rollno == LAST->Mohamadwahyu) {
		return true;
	}
	else {
		return false;
...
void CircularLinkedList::traverse() {
	if (listEmpty()) {
		...
		while (currentNode != LAST) {
			cout << currentNode->Mohamadwahyu << " " << currentNode->Rizkiadnan << endl;
			currentNode = currentNode->next;
		}
		cout << LAST->Mohamadwahyu << " " << LAST->Rizkiadnan << endl;
	}
}
int main() {
